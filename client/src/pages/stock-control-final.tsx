...
export default function StockControlFinal() {
  ...
  useEffect(() => {
    if (sessionItems && sessionItems.length > 0) {
      ...
      const recoveredItems: ScannedItem[] = sessionItems.map((item: any) => ({
        imei: item.imei,
        product: item.product || productArray.find(p => p.imei === item.imei),
        timestamp: new Date(item.scannedAt),
        status: item.product ? 'found' : 'not_found'
      }));
  ...
  const pauseScanningSession = async () => {
    if (currentSession) {
      try {
        const response = await fetch(`/api/stock-control/sessions/${currentSession.id}`, {
          ...
          body: JSON.stringify({
            status: 'paused',
            scannedProducts: scannedItems.length
          }),
        });

        if (response.ok) {
          console.log('⏸️ Session paused successfully');
        }
        ...
        toast({
          ...
          description: `Progreso guardado: ${scannedItems.length}/${sessionStartCounts.total} productos`,
        });
      } catch (error) {
        console.error('❌ Error pausing session:', error);
        toast({
          title: "❌ Error",
          description: "No se pudo pausar la sesión",
          variant: "destructive",
        });
      }
    }
      ...
      if (!alreadyScanned) {
        try {
          ...
          if (response.ok) {
            ...
            console.log('✅ Product scanned successfully:', scannedProduct);
          } else {
            const errorData = await response.text();
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-3">
                    ...
                    {!isScanning && !currentSession ? (
                      ...
                      <div className="flex gap-2">
                        {!isPaused ? (
                          <Button onClick={pauseScanningSession} className="bg-yellow-600 hover:bg-yellow-700">
